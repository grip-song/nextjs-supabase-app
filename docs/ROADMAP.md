...
- **Task 003: 공통 컴포넌트 라이브러리 구현** ✅ - 완료
  ...
  - ✅ 로딩 스켈레톤 컴포넌트 구현
  - ✅ 빈 상태 UI 컴포넌트 구현
  - ✅ 더미 데이터 생성 및 관리 유틸리티 작성 — Task 007~011에서 실 Supabase 연동으로 순차 대체되어 Task 011에서 `lib/dummy-data/` 전체 삭제됨
...
- **Task 007: 데이터베이스 스키마 및 Supabase 초기 설정** ✅ - 완료
  ...
  - **참고**: 신규 가입자용 프로필 자동 생성 트리거가 스타터킷 잔재인 `public.profiles`를 대상으로 하고 있던 결함을 발견해 `public.users`로 교체함(Task 008 준비 과정에서 수정)
...
- **Task 008: 인증 시스템 및 권한 관리** ✅ - 완료
  ...
  - **참고**: 관리자 판정이 위조 가능한 `auth.user_metadata.role`을 사용하던 보안 결함을 발견해 신뢰 가능한 `public.users.role`/`is_admin()` 기반으로 교체하고, admin 레이아웃에 서버 사이드 이중 검증을 추가함
...
- **Task 009: 이벤트 CRUD 및 초대 시스템** ✅ - 완료
  ...
  - 초대 링크 공유 기능 구현 (카카오톡, 클립보드) (F003)
    - ✅ 클립보드 복사
    - ~~카카오톡 공유~~ — 범위 제외(사용자 확인). 이번 프로젝트에서 불필요한 것으로 확정되어 구현하지 않음
  - ✅ 이벤트 상태 자동 관리 로직 구현 (F008) — DB 컬럼 동기화 대신 event_date 기준 순수 함수(getEventStatus)로 렌더링 시점마다 계산하는 방식으로 구현
  - ✅ Playwright MCP를 활용한 이벤트 생성/수정/삭제 테스트
  - **참고**: `event_participants`에 의존하는 화면(참여 중인 이벤트 탭, 참여자 목록/카운트, 초대 참여 액션)은 Task 010 범위로 남겨두고 더미 데이터를 유지함
...
- **Task 010: 참여자 관리** ✅ - 완료
  ...
  - ✅ 이벤트 나가기(참여 취소) 기능 구현 — 사용자 피드백으로 범위에 추가됨(호스트는 나갈 수 없고 이벤트 삭제로만 정리 가능)
  - ✅ Playwright MCP를 활용한 실시간 참여자 업데이트 테스트
  - **참고**: E2E 테스트 중 `event_participants`의 REPLICA IDENTITY가 기본값(PK만 포함)이라 나가기(DELETE) 실시간 이벤트가 postgres_changes 필터에 매칭되지 않아 구독자에게 전달되지 않는 버그를 발견, `replica identity full`로 마이그레이션해 수정함(Task 007에서는 Realtime publication 등록만 확인했던 부분)

- **Task 011: 관리자 대시보드 백엔드 구현** ✅ - 완료
  - ✅ 대시보드 지표 집계 쿼리 구현 (F012)
  - ✅ 이벤트 관리 테이블 검색/필터/삭제 API 구현 (F013)
  - ✅ 사용자 관리 테이블 검색/필터 API 구현 (F014) — 사용자 삭제는 범위 제외(아래 참고)
  - ✅ 통계 데이터 집계 및 그래프 데이터 API 구현 (F015)
  - ✅ 페이지네이션 및 정렬 로직 구현
  - ✅ Playwright MCP를 활용한 관리자 기능 통합 테스트
  - **참고**: `public.users`에는 admin delete RLS 정책이 없고 `auth.users`와 1:1 트리거로 연결되어 있어, 완전한 사용자 삭제는 `supabase.auth.admin.deleteUser()`(서비스 롤 키 필요)로만 가능함. 레포에 서비스 롤 키 도입처가 없어 과도한 엔지니어링으로 판단, 사용자 삭제 기능은 이번 태스크 범위에서 제외하고 관리자 사용자 테이블에서 삭제 버튼/Dialog를 제거함
  - **참고**: 이벤트 삭제는 `app/actions/admin.ts`의 `deleteEventAsAdmin`(관리자 사전 체크 + `events_admin_all` RLS를 최종 방어선)으로 기존 `deleteEvent`와 분리 구현함
  - **참고**: 이번 태스크에서 `lib/dummy-data/` 디렉터리 전체가 더 이상 어디서도 참조되지 않음을 확인해 삭제함(관리자 화면뿐 아니라 프로젝트 전역에서 마지막 잔재였음)
